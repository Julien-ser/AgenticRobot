...
float Get_Sonar(void) {
  ...
  float distance;
  // Ensure TRIG is LOW before pulse (HC-SR04 requires clean LOW→HIGH→LOW)
  digitalWrite(PIN_SONIC_TRIG, LOW);
  delayMicroseconds(2);
  digitalWrite(PIN_SONIC_TRIG, HIGH);  // make trigPin output high level lasting for 10μs to trigger HC_SR04
